...
Public Class frm_mesero
    ...
    Public Function existeHabilitado(ByVal Cad As String) As Boolean
        ...
        MConexion.Conectar()
        Lector = MConexion.EjecutarConsulta("Proc_BuscarMesero'    " + (Cad) + " '")
    ...
    Public Function existeDeshabilitado(ByVal Cad As String) As Boolean
        Dim Lector As SqlDataReader
        MConexion.Conectar()
        Lector = MConexion.EjecutarConsulta("Proc_BuscarMeseroDeshabilitado'    " + (Cad) + " '")
        If (Lector.HasRows()) Then
            Return True
        Else
            Return False
        ...
        If (ch_deshabilitado.Checked = False) Then
            If ((tieneDatosCi() = True) And (EsNumero(tf_ci.Text) = True)) Then
                If (existeHabilitado(tf_ci.Text) = True) Then
                    Dim Lector As SqlDataReader
                    MConexion.Conectar()
                    Lector = MConexion.EjecutarConsulta("Proc_BuscarMesero '    " + (tf_ci.Text) + " '")
                    Lector.Read()
                ...
                Else
                    MsgBox("NO EXISTE EL MESERO A BUSCAR")
                End If
            Else
        ...
        Else
            If ((tieneDatosCi() = True) And (EsNumero(tf_ci.Text) = True)) Then
                If (existeDeshabilitado(tf_ci.Text) = True) Then
                    Dim Lector As SqlDataReader
                    MConexion.Conectar()
                    Lector = MConexion.EjecutarConsulta("Proc_BuscarMeseroDeshabilitado'    " + (tf_ci.Text) + " '")
                    ...
                    MConexion.Desconectar()
                Else
                    MsgBox("NO EXISTE EL MESERO DESHABILITADO A BUSCAR")
                End If
            Else
                MsgBox("RELLENAR CASILLAS, CI DE TIPO NUMERICO")
            End If
    ...
    Private Sub bt_guardar_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt_guardar.Click
        If (ch_deshabilitado.Checked = False) Then
            If ((tieneDatos() = True) And (EsNumero(tf_ci.Text) = True)) Then
                If (existeHabilitado(tf_ci.Text) = False And existeDeshabilitado(tf_ci.Text) = False) Then
                    ...
                    m.MESERO_ID = CInt(tf_ci.Text)

                    MConexion.Conectar()
                    u.Guardar()
                    m.Guardar()
                    MConexion.Desconectar()
                    MsgBox("SE GUARDAR EXITOSAMENTE EL MESERO")

                    'LIMPIA EL EDIT CI (refresh)
                    tf_ci.Items.Clear()

                    'LISTA ACTUALIZADA DE ADMINISTRADORES HABILITADO EN EDIT TF_CI
                    MConexion.Conectar()
                    M_Generico.CargarCombo(tf_ci, "Proc_ListarMesero")
                    MConexion.Desconectar()
                Else
                    MsgBox("NO SE PUEDE GUARDAR, YA EXISTE EL MESERO")
                End If
            Else
                MsgBox("RELLENAR CASILLAS, CI DE TIPO NUMERICO")
            End If
        Else
            If ((tf_ci.Text <> "") And (EsNumero(tf_ci.Text) = True)) Then
                If (existeDeshabilitado(tf_ci.Text) = True) Then
                    MConexion.Conectar()
                    MConexion.EjecutarConsulta("Proc_GuardarMeseroDeshabilitado '    " + (tf_ci.Text) + " '")
                    MConexion.Desconectar()
                    MsgBox("SE HABILITO EXISTOSAMENTE EL MESERO")

                    'LIMPIA EL EDIT CI (refresh)
                    tf_ci.Items.Clear()

                    'LISTA ACTUALIZADA DE ADMINISTRADORES DESHABILITADOS EN EDIT TF_CI
                    MConexion.Conectar()
                    M_Generico.CargarCombo(tf_ci, "Proc_ListarMeseroDeshabilitado")
                    MConexion.Desconectar()
                Else
                    MsgBox("NO SE PUEDE HABILITAR, NO EXISTE EL MESERO")
                End If
            Else
                MsgBox("RELLENAR CASILLAS, CI DE TIPO NUMERICO")
            End If
    ...
    Private Sub bt_modificar_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt_modificar.Click
        If ((tf_ci.Text <> "") And (EsNumero(tf_ci.Text) = True)) Then
            If (existeHabilitado(tf_ci.Text) = True) Then
                ...
                End If
            Else
                MsgBox("NO SE PUEDE MODIFICAR, NO EXISTE EL MESERO")
            End If
        Else
            MsgBox("RELLENAR CASILLAS, CI DE TIPO NUMERICO")
        End If
    ...
    Private Sub bt_eliminar_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt_eliminar.Click
        If ((tf_ci.Text <> "") And (EsNumero(tf_ci.Text) = True)) Then
            If (existeHabilitado(tf_ci.Text) = True) Then
                Dim Lector As SqlDataReader
                MConexion.Conectar()
                Lector = MConexion.EjecutarConsulta("Proc_EliminarMesero'    " + (tf_ci.Text) + " '")
                MConexion.Desconectar()
                MsgBox("MESERO ELIMINADO EXITOSAMENTE")

                'LIMPIA EL EDIT CI (refresh)
                tf_ci.Items.Clear()

                'LISTA ACTUALIZADA DE MESERO HABILITADO EN EDIT TF_CI
                MConexion.Conectar()
                M_Generico.CargarCombo(tf_ci, "Proc_ListarMesero")
                MConexion.Desconectar()
            Else
                MsgBox("NO SE PUEDE ELIMINAR, NO EXISTE EL MESERO")
            End If
        Else
    ...
    Private Sub bt_mostrar_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt_mostrar.Click
        If (ch_deshabilitado.Checked = True) Then
            MConexion.Conectar()
            M_Generico.CargarGrid(dg_mostrar, "Proc_MostrarMeseroDeshabilitado", "tbl_MESERO")
            MConexion.Desconectar()
        Else
            MConexion.Conectar()
            M_Generico.CargarGrid(dg_mostrar, "Proc_MostrarMesero", "tbl_MESERO")
            MConexion.Desconectar()
        End If
    End Sub
    ...
    Private Sub ch_deshabilitado_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ch_deshabilitado.CheckedChanged
        If (ch_deshabilitado.Checked = True) Then
            ...
            'LISTA LOS MESERO DESHABILITADOS EN EDIT TF_CI
            MConexion.Conectar()
            M_Generico.CargarCombo(tf_ci, "Proc_ListarMeseroDeshabilitado")
            MConexion.Desconectar()
        Else
            ...
            'LISTA LOS MESERO HABILITADOS EN EDIT TF_CI
            MConexion.Conectar()
            M_Generico.CargarCombo(tf_ci, "Proc_ListarMesero")
            MConexion.Desconectar()
        End If
