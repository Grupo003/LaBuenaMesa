...
Public Class frm_administrador
    ...
    Public Function existeHabilitado(ByVal Cad As String) As Boolean
        ...
        MConexion.Conectar()
        Lector = MConexion.EjecutarConsulta("Proc_BuscarAdministrador'    " + (Cad) + " '")
    ...
    Public Function existeDeshabilitado(ByVal Cad As String) As Boolean
        Dim Lector As SqlDataReader
        MConexion.Conectar()
        Lector = MConexion.EjecutarConsulta("Proc_BuscarAdministradorDeshabilitado'    " + (Cad) + " '")
        If (Lector.HasRows()) Then
            Return True
        Else
            Return False
        ...
        If (ch_deshabilitado.Checked = False) Then
            If ((tieneDatosCi() = True) And (EsNumero(tf_ci.Text) = True)) Then
                If (existeHabilitado(tf_ci.Text) = True) Then
                    Dim Lector As SqlDataReader
                    MConexion.Conectar()
                    Lector = MConexion.EjecutarConsulta("Proc_BuscarAdministrador '    " + (tf_ci.Text) + " '")
                    Lector.Read()
        ...
        Else
            If ((tieneDatosCi() = True) And (EsNumero(tf_ci.Text) = True)) Then
                If (existeDeshabilitado(tf_ci.Text) = True) Then
                    Dim Lector As SqlDataReader
                    MConexion.Conectar()
                    Lector = MConexion.EjecutarConsulta("Proc_BuscarAdministradorDeshabilitado'    " + (tf_ci.Text) + " '")
                    ...
                    MConexion.Desconectar()
                Else
                    MsgBox("NO EXISTE EL ADMINISTRADOR DESHABILITADO A BUSCAR")
                End If
            Else
                MsgBox("RELLENAR CASILLAS, CI DE TIPO NUMERICO")
            End If
        ...
        If (ch_deshabilitado.Checked = False) Then
            If ((tieneDatos() = True) And (EsNumero(tf_ci.Text) = True)) Then
                If (existeHabilitado(tf_ci.Text) = False And existeDeshabilitado(tf_ci.Text) = False) Then
                    ...
                    Dim a As New cls_ADMINISTRADOR
                    Dim au As New cls_ACCIONES_USUARIO
                    ...
                    a.Guardar()
                    MConexion.Desconectar()
                    MsgBox("SE GUARDAR EXITOSAMENTE EL ADMINISTRADOR")
                    ...
                    MConexion.Conectar()
                    M_Generico.CargarCombo(tf_ci, "Proc_ListarAdministrador")
                    MConexion.Desconectar()
                Else
                    MsgBox("NO SE PUEDE GUARDAR, YA EXISTE EL ADMINISTRADOR")
                End If
            Else
                MsgBox("RELLENAR CASILLAS, CI DE TIPO NUMERICO")
            End If
        Else
            If ((tf_ci.Text <> "") And (EsNumero(tf_ci.Text) = True)) Then
                If (existeDeshabilitado(tf_ci.Text) = True) Then
                    ...
                    MConexion.Desconectar()
                    MsgBox("SE HABILITO EXISTOSAMENTE EL ADMINISTRADOR")
                    ...
                    MConexion.Conectar()
                    M_Generico.CargarCombo(tf_ci, "Proc_ListarAdministradorDeshabilitado")
                    MConexion.Desconectar()
                Else
                    MsgBox("NO SE PUEDE HABILITAR, NO EXISTE EL ADMINISTRADOR")
                End If
            Else
                MsgBox("RELLENAR CASILLAS, CI DE TIPO NUMERICO")
            End If
    ...
    Private Sub bt_modificar_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt_modificar.Click
        ...
        If ((tf_ci.Text <> "") And (EsNumero(tf_ci.Text) = True)) Then
            If ((existeHabilitado(tf_ci.Text) = True)) Then
                ...
                End If
            Else
                MsgBox("NO SE PUEDE MODIFICAR, NO EXISTE EL ADMINISTRADOR")
            End If
        Else
            MsgBox("RELLENAR CASILLAS, CI DE TIPO NUMERICO")
        End If
    ...
    Private Sub bt_eliminar_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt_eliminar.Click
        If ((tf_ci.Text <> "") And (EsNumero(tf_ci.Text) = True)) Then
            If (existeHabilitado(tf_ci.Text) = True) Then
                Dim Lector As SqlDataReader
                MConexion.Conectar()
                Lector = MConexion.EjecutarConsulta("Proc_EliminarAdministrador'    " + (tf_ci.Text) + " '")
                MConexion.Desconectar()
                MsgBox("ADMINISTRADOR ELIMINADO EXITOSAMENTE")
                ...
                MConexion.Desconectar()
            Else
                MsgBox("NO SE PUEDE ELIMINAR, NO EXISTE EL ADMINISTRADOR")
            End If
        Else
    ...
    Private Sub bt_mostrar_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt_mostrar.Click
        If (ch_deshabilitado.Checked = True) Then
            MConexion.Conectar()
            M_Generico.CargarGrid(dg_mostrar, "Proc_MostrarAdministradorDeshabilitado", "tbl_ADMINISTRADOR")
            MConexion.Desconectar()
        Else
            MConexion.Conectar()
            M_Generico.CargarGrid(dg_mostrar, "Proc_MostrarAdministrador", "tbl_ADMINISTRADOR")
            MConexion.Desconectar()
        End If
    ...
    Private Sub frm_administrador_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'LISTA LOS GRUPOS EN EDIT TF_GRUPO
        MConexion.Conectar()
